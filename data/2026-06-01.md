...
TL;DR: This paper presents a 256x128 reconfigurable SRAM analog computing-in-memory (CIM) array that supports 1–7b inputs, 2–4b weights (including ternary via a dual-8T cell), and 1–7b outputs. It introduces a highly compact in-memory ADC (IMADC) with only 3% area overhead (9× smaller than prior IMADCs), a BSCHA scheme that cuts multi-bit input latency by 1.9× vs PWM and 6.6× vs bit-slicing, and a dual-8T bitcell plus read-wordline under-driven cascode (RWLD) that improves unit discharge current linearity 7× and extends usable read bitline voltage 3.5×.


<details>
  <summary>Details</summary>
Motivation: SRAM-based analog CIM is energy- and area-efficient but is limited by large ADC overhead, high latency for multi-bit inputs, and constrained/read-bitline voltage and linearity that limit precision and dynamic range.

Method: System-level: design of a reconfigurable 256×128 array supporting variable input/weight/output bitwidths. ADC: compact IMADC architecture optimized for area. Timing/compute: BSCHA (bit-slice/shot-combine/ hierarchical accumulation — a hybrid that reduces cycles vs PWM/bit-slicing). Circuit/cell: a dual-8T bitcell with decoupled read path to store ternary weights and an RWLD cascode bias to increase read headroom and linearize discharge current.

Result: Reported IMADC area overhead = 3% (9× area reduction vs prior IMADCs). BSCHA latency = 1.9× lower than PWM and 6.6× lower than conventional bit-slicing. Dual-8T with RWLD gives 7× improvement in unit current linearity and 3.5× increase in usable read BL voltage.

Conclusion: The combined architecture and circuit techniques materially reduce ADC area and multi-bit latency while improving read linearity and dynamic range, enabling practical multi-bit analog SRAM CIM with ternary-weight support and better system-level efficiency.
...
### [6] [GaMi: Geometry-Agnostic Material Identification via Cross-Modal Subtractive Disentanglement](https://arxiv.org/abs/2605.30818)
...
### [7] [Compact and Energy-Efficient Memristive Spiking Neuromorphic Accelerator for Bio-inspired Interception Tasks](https://arxiv.org/abs/2605.31141)
...
### [8] [BERS: Locally Optimal Continuous Algorithm for Maritime Weather Routing with Just-in-Time Arrival](https://arxiv.org/abs/2605.31533)
